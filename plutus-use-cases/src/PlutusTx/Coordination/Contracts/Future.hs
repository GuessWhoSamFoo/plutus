...
module PlutusTx.Coordination.Contracts.Future(
    -- $future
      Future(..)
    , FutureAccounts(..)
    , mkAccounts
...
import           PlutusTx.Coordination.Contracts.TokenAccount (Account (..))
...
--   2. Using oracle values to obtain current pricing information (see note [Oracles] in PlutusTx.Coordination.Contracts)
...
[Oracles] in PlutusTx.Coordination.Contracts. Also note that we
